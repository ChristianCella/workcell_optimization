...
def main():
    ...
    y_values = [-0.2, 0.0, 0.2]
    discretizations = [1, 1, 90]
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        ...
        for d in range(3):
            ...
            input(f"Evaluation for point {d}. Press enter")
            ...
            t_w_p = np.array([2.6, y_values[d], 0.2]) # [0.2, 0.2, 0.2]
            ...
            sols_ok, fk_ok = [], []
            for i in range(discretizations[d]): # 0, 1, 2, ... N_disc-1
                R_w_p_rotated = R.from_euler('XYZ', [theta_w_p_x_0, theta_w_p_y_0, theta_w_p_z_0 + i * 2 * np.pi / discretizations[d]], degrees=False).as_matrix()
            ...
            if params.use_ikflow:
                ...
                q = np.radians([-90, -90, -90, -90, 90, 90])
                data.qpos[:6] = q.tolist()
                mujoco.mj_forward(model, data)

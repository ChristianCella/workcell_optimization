...
from parameters import ScrewingRandom, Ur5eRobot
parameters = ScrewingRandom()
            ...
            for j in range(len(ref_body_ids)): # ! For each piece to be screwed
                ...
                if len(sols_np) > 0: #! There are IK solutions available
                    ...
                    for i, (q, x) in enumerate(zip(sols_np, fk_np), 1):
                        ...
                        data.qpos[:robot_parameters.nu] = q
                        mujoco.mj_forward(model, data)
                        #viewer.sync()
                        ...
                        #print(f"Number of collisions: {n_cols}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        for gen in range(n_iter): #! Loop until the maximum number of generations is reached
            ...
            rng = np.random.default_rng(getattr(parameters, "seed", None))

            # Uniform samples in the scaled box [-1, 1]^d
            zs = [rng.uniform(-1.0, 1.0, size=14) for _ in range(popsize)]
            ...
            for idx, sol in enumerate(sols): #! For each individual

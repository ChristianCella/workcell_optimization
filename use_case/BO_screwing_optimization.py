...
'''
# implementazione sigmoide esplorazione variabile
def psi(iteration, kappa, it_center, num_iters):
    cen = it_center * num_iters
    return kappa / (1 + np.exp(0.1 * (iteration - cen)))

  # Adjust the sigmoid parameters as needed
...
def EI(X, GPR_model, best_y,iteration,kappa, it_center, num_iters):
    ...
    improvement = mu - best_y - psi(iteration, kappa, it_center, num_iters) #! ps is the exploration variable
    ...
    if debug:
        ...
        print(f"xi        : {psi(iteration, kappa, it_center, num_iters):.6f}")
        print(f"improv.   : {improvement}")
        print(f"sigma     : {sigma}")
        print(f"z         : {z}")
        print(f"EI        : {ei}")
        print("--------------------\n")
...
def optimize_acquisition(GPR_model, n, anchor_number, best_evaluation, x_inf, x_sup, constraint, kappa,iteration, it_center, num_iters):
    ...
    acquisition_values = EI(random_points, GPR_model, best_evaluation,iteration,kappa, it_center, num_iters) # I apply the EI acquisition function to these points
    ...
    for anchor in selected_anchors :
        ...
        acq = lambda anchor, GPR_model: EI(anchor, GPR_model, best_evaluation,iteration,kappa, it_center, num_iters)
    ...
    optimized_acquisition_values = EI(optimized_points, GPR_model, best_evaluation,iteration,kappa, it_center, num_iters)
...
if __name__ == "__main__":
    ...
    verbose = True
    need_training = False
    it_center=0.7 # Center of the sigmoid for the exploration variable
    debug=False
...
''' 
    ...
    for i in range(num_iters): # 0, 1, ..., num_iters-1
        ...
        x_next_s,EI_S = optimize_acquisition(GP, n, anchor_number, best_evaluation, x_inf_s, x_sup_s, constraint, kappa,i, it_center, num_iters)
        EI_history.append(EI_S) # Store the EI value for this iteration
...
------------------------------SALVATAGGIO DEI RISULTATI------------------------------------      '''
    ...
    if verbose:
        ...
        # Save the history of evaluations
    EI_csv_path = os.path.join(base_dir, "datasets", f"EI_{num_iters}_EI_2.csv")
    df_history_ei = pd.DataFrame({"iteration": range(1, len(EI_history) + 1), "EI":   EI_history})
    df_history_ei.to_csv(EI_csv_path, index=False)
    if verbose:
        print(f"History of evaluations saved to: {EI_csv_path}")
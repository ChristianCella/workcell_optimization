...
try:
    import sys
    sys.path.append('/Users/alessandrocasciani/Desktop/PYTHON_MUJOCO/TuRBO')  # Ensure this path is correct
    from turbo.turbo_m import TurboM
except Exception as e:
    raise ImportError(
        "Could not import turbo.turbo_m.TurboM. "
        "Clone https://github.com/uber-research/TuRBO and ensure its 'turbo/' "
    ...
    def run_simulation(params: np.ndarray) -> float:
        ...
        if n_cols_initial > 0:
            ...
            for j in range(len(ref_body_ids)):
                norms.append(1e3) #era 1e12 per il discorso di non corrompere in gpr l'ho messo a 200 ora a 20
            ...
            fit_path = 1e3 / (2 * np.pi * 0.85) #era 1e12 stesso discorso di prima
            ...
            for j in range(len(ref_body_ids)): # ! For each piece to be screwed
                ...
                if not np.array_equal(best_q, np.zeros(6)):
                    ...
                    norms.append(1e3) #stesso discorso#! Ik not feasible => Drive the algorithm away from this configuration
